...
(function (global, factory) {
  ...
  function extend(target, source) {
    let index; let length; let key; let
      sourceKeys;
  ...
  function constructYamlTimestamp(data) {
    ...
    let delta = null; let tz_hour; let tz_minute; let
      date;
  ...
  function resolveYamlBinary(data) {
    ...
    let code; let idx; let bitlen = 0; const max = data.length; const
      map = BASE64_MAP;
  ...
  function resolveYamlOmap(data) {
    ...
    const objectKeys = []; let index; let length; let pair; let pairKey; let pairHasKey;
    const object = data;
...
prefix;
  ...
  function captureSegment(state, start, end, checkJson) {
    let _position; let _length; let _character; let
_result;
  ...
  function mergeMappings(state, destination, source, overridableKeys) {
    let sourceKeys; let key; let index; let
quantity;
...
captureEnd;
  ...
  function composeNode(state, parentIndent, nodeContext, allowToSeek, allowCompact) {
    ...
    allowBlockStyles = allowBlockScalars = allowBlockCollections = CONTEXT_BLOCK_OUT === nodeContext
  ...
  function loadDocuments(input, options) {
    ...
    if (input.length !== 0) {
      ...
      if (input.charCodeAt(input.length - 1) !== 0x0A
          &&/* LF */ input.charCodeAt(input.length - 1) !== 0x0D/* CR */) {
        input += '\n';
      }
  ...
  function compileStyleMap(schema, map) {
    let result; let keys; let index; let length; let tag; let style; let
type;
...
type;
  ...
  function codePointAt(string, pos) {
    const first = string.charCodeAt(pos); let
second;
...
line = match[2];
  ...
  function foldLine(line, width) {
    ...
    let start = 0; let end; let curr = 0; let
next = 0;
    let result = '';
  ...
  function detectType(state, object, explicit) {
    let _result; let typeList; let index; let length; let type; let
style;
