...
export class YargsParser {
  ...
  parse(argsInput, options) {
    ...
    [].concat(opts.array || []).filter(Boolean).forEach((opt) => {
      ...
      const assignment = Object.keys(opt).map((key) => {
        const arrayFlagKeys = {
          boolean: 'bools',
          string: 'strings',
          number: 'numbers',
    ...
    [].concat(opts.number || []).filter(Boolean).forEach((key) => {
      flags.numbers[key] = true;
      flags.keys.push(key);
    });
    [].concat(opts.count || []).filter(Boolean).forEach((key) => {
      flags.counts[key] = true;
      flags.keys.push(key);
    });
    [].concat(opts.normalize || []).filter(Boolean).forEach((key) => {
    ...
    function processValue(key, val) {
      ...
      if (checkAllAliases(key, flags.normalize) && checkAllAliases(key, flags.arrays)) {
        if (Array.isArray(val)) value = val.map((val) => mixin.normalize(val));
    ...
    function extendAliases(...args) {
      args.forEach((obj) => {
        Object.keys(obj || {}).forEach((key) => {
          ...
          flags.aliases[key].concat(key).forEach((x) => {
          ...
          flags.aliases[key].forEach((x) => {
            flags.aliases[x] = [key].concat(flags.aliases[key].filter((y) => x !== y));
          });
        });
      });
    }
