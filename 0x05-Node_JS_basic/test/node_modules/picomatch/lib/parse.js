...
const parse = (input, options) => {
  ...
  const extglobOpen = (type, value) => {
    ...
    push({
      type: 'paren', extglob: true, value: advance(), output,
    });
    extglobs.push(token);
  };
  ...
  const extglobClose = (token) => {
    ...
    push({
      type: 'paren', extglob: true, value, output,
    });
    decrement('parens');
  };
  ...
  if (opts.fastpaths !== false && !/(^[*!]|[/()[\]{}"])/.test(input)) {
    ...
    if (backslashes === true) {
      if (opts.unescape === true) {
        ...
        output = output.replace(/\\+/g, (m) => (m.length % 2 === 0 ? '\\\\' : (m ? '\\' : '')));
      }
    }
  ...
  while (!eos()) {
    ...
    if (value === '@') {
      if (opts.noextglob !== true && peek() === '(' && peek(2) !== '?') {
        push({
          type: 'at', extglob: true, value, output: '',
        });
        continue;
      }
